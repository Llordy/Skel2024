...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            self.__Grid = []
            self.__GameDifficulty = int(input("Enter Game Difficulty (1 or 2): "))
            while self.__GameDifficulty != 1 and self.__GameDifficulty != 2:
                self.__GameDifficulty = input("Invalid Input. Enter Game Difficulty (1 or 2): ")
            if self.__GameDifficulty == 1:
    ...
    def __LoadPuzzle(self, Filename):
        try:
            ...
            print("Puzzle not loaded, re-enter file name in the next line")
            Main()
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if Symbol.upper() == 'B' and CurrentCell.GetSymbol() == "@":
                CurrentCell.ChangeSymbolInCell('-')

            elif Symbol.upper() == "B":
                print("The cell you have chosen is not blocked")
            else:
        ...
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    ...
                    PatternString += self.__GetCell(StartRow, StartColumn + 1).GetSymbol()#top middle
                    PatternString += self.__GetCell(StartRow, StartColumn + 2).GetSymbol()#top right
                    PatternString += self.__GetCell(StartRow - 1, StartColumn + 2).GetSymbol()#middle right
                    PatternString += self.__GetCell(StartRow - 2, StartColumn + 2).GetSymbol()#bottom right
                    PatternString += self.__GetCell(StartRow - 2, StartColumn + 1).GetSymbol()#bottom middle
                    ...
                    PatternString += self.__GetCell(StartRow - 1, StartColumn + 1).GetSymbol()#middle middle
                    RotatedPatternString = ""
                    RotatedPatternString += self.__GetCell(StartRow, StartColumn + 2).GetSymbol()  # top right
                    RotatedPatternString += self.__GetCell(StartRow, StartColumn + 1).GetSymbol()#top middle
                    RotatedPatternString += self.__GetCell(StartRow, StartColumn).GetSymbol()  # top left
                    RotatedPatternString += self.__GetCell(StartRow - 1, StartColumn).GetSymbol()  # middle left
                    RotatedPatternString += self.__GetCell(StartRow - 2, StartColumn).GetSymbol()  # bottom left
                    RotatedPatternString += self.__GetCell(StartRow - 2, StartColumn + 1).GetSymbol()  # bottom middle
                    RotatedPatternString += self.__GetCell(StartRow - 2, StartColumn + 2).GetSymbol()#bottom right
                    RotatedPatternString += self.__GetCell(StartRow - 1, StartColumn + 2).GetSymbol()  # middle right
                    RotatedPatternString += self.__GetCell(StartRow - 1, StartColumn + 1).GetSymbol()#middle middle
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            ...
                            return 10
                        if P.MatchesRotatedPattern(RotatedPatternString, CurrentSymbol):
                            self.__GetCell(StartRow, StartColumn).AddToNotAllowedSymbols(CurrentSymbol)
                            self.__GetCell(StartRow, StartColumn + 1).AddToNotAllowedSymbols(CurrentSymbol)
                            self.__GetCell(StartRow, StartColumn + 2).AddToNotAllowedSymbols(CurrentSymbol)
                            self.__GetCell(StartRow - 1, StartColumn + 2).AddToNotAllowedSymbols(CurrentSymbol)
    ...
    def __GetSymbolFromUser(self):
        Symbol = ""
        while (not Symbol in self.__AllowedSymbols) and (Symbol != 'B'):
            Symbol = input("Enter symbol or enter 'B' to blow up the cell: ").upper()
        return Symbol
...
class Pattern():
    def __init__(self, SymbolToUse, PatternString, RotatedPatternString):
        self.__Symbol = SymbolToUse
        self.__PatternSequence = PatternString
        self.__RotatedPatternSequence = RotatedPatternString
    ...
    def MatchesRotatedPattern(self, PatternString, SymbolPlaced):
        if SymbolPlaced != self.__Symbol:
            return False
        for Count in range(0, len(self.__PatternSequence)):
            try:
